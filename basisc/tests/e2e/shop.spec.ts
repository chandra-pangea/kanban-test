...
test.describe("Demo shop E2E", () => {
  test("Scenario 1: product browsing — products are visible", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("Scenario 2: filtering — category, search, and results", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("Scenario 3: add to cart — badge updates and persists on refresh", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("Scenario 5: checkout — success and cart cleared", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("Scenario 6a: empty cart UI", async ({ page }) => {
    await page.goto("/cart");
    await expect(page.getByTestId("cart-empty")).toBeVisible();
    await expect(page.getByText("Your cart is empty")).toBeVisible();
  });

  test("Scenario 6b: invalid filter shows no products message", async ({
    page,
  }) => {
    await page.goto("/");
